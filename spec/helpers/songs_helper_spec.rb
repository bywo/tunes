...
describe SongsHelper do
  ...
  describe "song_from_youtube_url" do
    before do
      @video_info = OpenStruct.new({
        title: 'Cool Song',
      ...
      YouTubeIt::Client.any_instance.stub(:video_by).and_return(@video_info)
    end

    it "creates a song and sound with correct metadata" do
      song = helper.song_from_youtube_url('cool.url/bro')
      expect(song[:title]).to eq(@video_info.title)
      expect(song[:artist]).to eq(@video_info.author.name)
      expect(song.sounds.exists?({sound_type: 'youtube', location: @video_info.unique_id})).to eq(true)
    end

    describe "redundant add" do
      before { @song = helper.song_from_youtube_url('blah') }

      it "should return same song" do
        song = helper.song_from_youtube_url('blah')
        expect(song).to eq(@song)
      end
    end
  end
